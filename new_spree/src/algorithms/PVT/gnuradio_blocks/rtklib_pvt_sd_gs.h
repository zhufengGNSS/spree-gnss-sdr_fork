...
#ifndef GNSS_SDR_RTKLIB_PVT_SD_GS_H
#define GNSS_SDR_RTKLIB_PVT_SD_GS_H
...
class Rtcm_Printer;
class Rtklib_Solver;
class rtklib_pvt_sd_gs;

using rtklib_pvt_sd_gs_sptr = boost::shared_ptr<rtklib_pvt_sd_gs>;
 ...
 */
class rtklib_pvt_sd_gs : public gr::sync_block
{
private:
    friend rtklib_pvt_sd_gs_sptr rtklib_make_pvt_gs(uint32_t nchannels,
        const Pvt_Conf& conf_,
        const rtk_t& rtk);

    rtklib_pvt_sd_gs(uint32_t nchannels,
        const Pvt_Conf& conf_,
        const rtk_t& rtk);
...
public:
    ~rtklib_pvt_sd_gs();  //!< Default destructor
