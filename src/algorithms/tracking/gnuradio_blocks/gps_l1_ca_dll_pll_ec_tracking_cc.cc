/*!
 * \file gps_l1_ca_dll_pll_ec_tracking_cc.cc
...
#include "gps_l1_ca_dll_pll_ec_tracking_cc.h"
#include <cmath>
#include <iostream>
#include <memory>
#include <sstream>
...
gps_l1_ca_dll_pll_ec_tracking_cc_sptr
gps_l1_ca_dll_pll_ec_make_tracking_cc(
        long if_freq,
        long fs_in,
...
{
    return gps_l1_ca_dll_pll_ec_tracking_cc_sptr(new Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc(if_freq,
            fs_in, vector_length, queue, dump, dump_filename, pll_bw_hz, dll_bw_hz, early_late_space_chips));
}



void Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc::forecast (int noutput_items,
...
Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc::Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc(
        long if_freq,
        long fs_in,
        unsigned int vector_length,
        boost::shared_ptr<gr::msg_queue> queue,
                ...
                gr::io_signature::make(1, 1, sizeof(Gnss_Synchro)))
{
    ...
    d_extra_early_code=(gr_complex*)volk_malloc(2*d_vector_length * sizeof(gr_complex),volk_get_alignment());
    d_extra_late_code=(gr_complex*)volk_malloc(2*d_vector_length * sizeof(gr_complex),volk_get_alignment());
    ...
    d_Extra_Early=(gr_complex*)volk_malloc(sizeof(gr_complex),volk_get_alignment());
    ...
    d_Extra_Late=(gr_complex*)volk_malloc(sizeof(gr_complex),volk_get_alignment());
...
void Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc::start_tracking()
{
    /*
...
void Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc::update_local_code()
{
    double tcode_chips;
    ...
    for (int i = 0; i < epl_loop_length_samples; i++)
        {
            associated_chip_index = 1 + round(fmod(tcode_chips - 2*d_early_late_spc_chips, code_length_chips));
            d_extra_early_code[i] = d_ca_code[associated_chip_index];
            tcode_chips = tcode_chips + code_phase_step_chips;
        }

    memcpy(d_early_code,&d_extra_early_code[early_late_spc_samples],d_current_prn_length_samples* sizeof(gr_complex));
    memcpy(d_prompt_code,&d_extra_early_code[early_late_spc_samples*2],d_current_prn_length_samples* sizeof(gr_complex));
    memcpy(d_late_code,&d_extra_early_code[early_late_spc_samples*3],d_current_prn_length_samples* sizeof(gr_complex));
    memcpy(d_extra_late_code,&d_extra_early_code[early_late_spc_samples*4],d_current_prn_length_samples* sizeof(gr_complex));
}




void Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc::update_local_carrier()
{
...
{
    d_dump_file.close();
    d_dump_vestigial.close();
    d_dump_signal.close();
    d_dump_signal_wo.close();

    volk_free(d_prompt_code);
    volk_free(d_late_code);
    volk_free(d_extra_late_code);
    volk_free(d_early_code);
    volk_free(d_extra_early_code);
    volk_free(d_carr_sign);
    volk_free(d_carr_wo);
    volk_free(d_Extra_Early);
    volk_free(d_Early);
    volk_free(d_Prompt);
    volk_free(d_Late);
    volk_free(d_Extra_Late);
...
int Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc::general_work (int noutput_items, gr_vector_int &ninput_items,
                    ...
                    if (d_enable_tracking == true)
                        {
                            ...
                            d_dump_vestigial.write((char*)&delta_, sizeof(float));
                            d_dump_vestigial.write((char*)&RT_, sizeof(float));
                            d_dump_vestigial.write((char*)&Extra_RT_, sizeof(float));
                            ...
                            for(int i = 0; i< d_current_prn_length_samples; i++)
                                {
                                    ...
                                    in_I_wo = (*din_wo).real();
                                    in_Q_wo = (*din_wo).imag();
                                    d_dump_signal.write((char*)&in_I, sizeof(float)); 
                                    d_dump_signal.write((char*)&in_Q, sizeof(float)); 
                                    d_dump_signal_wo.write((char*)&in_I_wo, sizeof(float)); 
                                    d_dump_signal_wo.write((char*)&in_Q_wo, sizeof(float)); 
                                    din++;
                                    din_wo++;
                                }
                        */
            ...
            if (d_dump_signal.is_open() == false)
                {
                    try
                    {
                            ...
                            LOG(INFO) << "Tracking signal dump enabled on channel " << d_channel << " Log file: " 
                                      << d_dump_signal_filename.c_str() << std::endl;
                    }
                    catch (std::ifstream::failure e)
                    {
            ...
            if (d_dump_signal_wo.is_open() == false)
                {
                    try
                    {
                            ...
                            LOG(INFO) << "Tracking signal dump enabled on channel " << d_channel << " Log file: " 
                                      << d_dump_signal_filename_wo.c_str() << std::endl;
                    }
                    catch (std::ifstream::failure e)
                    {
                            LOG(WARNING) << "channel " << d_channel << " Exception opening trk signal dump file " << e.what() << std::endl;
...
*/
            if (d_dump_vestigial.is_open() == false)
                {
                    try
                    {
                            d_dump_vestigial_filename.append(boost::lexical_cast<std::string>(d_channel));
                            d_dump_vestigial_filename.append(".dat");
                            d_dump_vestigial.exceptions (std::ifstream::failbit | std::ifstream::badbit);
                            d_dump_vestigial.open(d_dump_vestigial_filename.c_str(), std::ios::out | std::ios::binary);
                            LOG(INFO) << "Tracking vestigial dump enabled on channel " << d_channel << " Log file: " 
                                     << d_dump_vestigial_filename.c_str() << std::endl;
                    }
                    catch (std::ifstream::failure e)
                    {
                            LOG(WARNING) << "channel " << d_channel << " Exception opening trk signal dump file " << e.what() << std::endl;
...
void Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc::set_channel_queue(concurrent_queue<int> *channel_internal_queue)
{
    d_channel_internal_queue = channel_internal_queue;
}


void Gps_L1_Ca_Dll_Pll_Ec_Tracking_cc::set_gnss_synchro(Gnss_Synchro* p_gnss_synchro)
{
